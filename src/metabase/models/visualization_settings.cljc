...
(def ^:private visualization-settings-schema
  [:map
   [::column-settings {:optional true} map?]
   [::click-behavior {:optional true} click-behavior-schema]])
...
(mu/defn db->norm-column-ref :- column-ref-schema
  ...
  (let [parsed (malli/parse db-column-ref-vec-schema column-ref-vec)]
    (if (= parsed ::malli/invalid)
      (throw (ex-info "Invalid input" (malli/explain db-column-ref-vec-schema column-ref-vec)))
      (let [[m parts] parsed]
        (case m
          :field
...
(mu/defn parse-db-column-ref :- column-ref-schema
  ...
  (let [parsed (malli/parse db-column-ref-schema column-ref)]
    (if (= parsed ::malli/invalid)
      (throw (ex-info "Invalid input" (malli/explain db-column-ref-schema column-ref)))
      (let [[k v]    parsed
            ref->vec (case k
